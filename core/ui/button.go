...
type Button struct {
	Rect image.Rectangle //大小
	Text string          //文本

	mouseDown bool //按下状态

	onPressed func(b *Button) //按下回调
}
...
func (b *Button) Draw(dst *ebiten.Image, uiImage *ebiten.Image) {
	t := imageTypeButton
	if b.mouseDown {
		t = imageTypeButtonPressed
	}
	drawNinePatches(dst, b.Rect, imageSrcRects[t], uiImage)
...
func drawNinePatches(dst *ebiten.Image, dstRect image.Rectangle, srcRect image.Rectangle, uiImage *ebiten.Image) {
	srcX := srcRect.Min.X
	srcY := srcRect.Min.Y
	srcW := srcRect.Dx()
	srcH := srcRect.Dy()
